...
class WebRenderer:
    ...
    def render(self) -> list[BoundingBox]:
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            self._render_a4_page(
                html_file, output_file=document_image, pdf_file=document_pdf
            )

            template_image = self.output_dir / "flat_template.png"
            self._render_a4_page(
                html_file,
                output_file=template_image,
            ...
            self._render_a4_page(
                html_file,
                output_file=information_delta_image,
                scripts=[
            ...
            return self._extract_template_fields(html_file=html_file)
    ...
    def _render_a4_page(
        ...
        def inner():
            ...
            base_scripts = [
                self.jquery_script,
                "$('head').prepend(\"<style>@font-face {font-family: 'customFont';src: url('file:///"
            ...
            print_options = {
                ...
                "marginBottom": 5
                / 25.4,  # set margin bottom to 5 mm -> keeps overflowing content on same page
    ...
    def _extract_template_fields(self, html_file: Path) -> list[BoundingBox]:
        ...
        self._render_a4_page(
            html_file,
            output_file=flat_template_structure_file,
            scripts=[
                "$('*').css('visibility', 'hidden');",
        ...
        color_reduce_map = [
            (insertion_color, container_color)
            for container_color, insertion_map in color_mapping.items()
            for insertion_color in insertion_map
        ]

        container_bboxes = BoundingBox.find_bounding_boxes(
            image=map_colors(forms_image, color_reduce_map),
            valid_colors=list(color_mapping.keys()),
            allow_disconnected=True,
        )

        insertion_colors = [
            color for insertion_map in color_mapping.values() for color in insertion_map
        ]
        insertion_names = [
            name
            for insertion_map in color_mapping.values()
            for name in insertion_map.values()
        ]

        insertion_bboxes = BoundingBox.find_bounding_boxes(
            image=forms_image, valid_colors=insertion_colors, bbox_names=insertion_names
        )
        ...
        return [
            ...
            for bbox in BoundingBox.expand_children(
                container_bbox, insertion_bboxes, occlusion_mask
            )
        ]
