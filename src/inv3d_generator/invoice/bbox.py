...
class BoundingBox:
    ...
    def intersects(self, other: "BoundingBox"):
        def intersects_1d(min1, max1, min2, max2):
            return max1 > min2 and max2 > min1

        return intersects_1d(self.x0, self.x1, other.x0, other.x1) and intersects_1d(
    ...
    def from_corners(
        ...
        return BoundingBox(
            top=y0, left=x0, height=y1 - y0, width=x1 - x0, name=name, color=color
        )

    @staticmethod
    def from_points(
        points: Iterable[Union[tuple, np.ndarray]],
        name: Optional[str] = None,
        color: Optional[tuple] = None,
    ):
    ...
    def find_bounding_boxes(
        ...
        def format_color(c):
            return tuple(c) if isinstance(c, IterableClass) else (c,)
        ...
        if bbox_names is None:
            ...
            name_map = {
                format_color(color): name
                for color, name in zip(valid_colors, bbox_names)
            }

        horizontal_lines = ~np.all(
            image[:-1, :-1] == image[1:, :-1], axis=-1
        )  # compare shifted images
        vertical_lines = ~np.all(
            image[:-1, :-1] == image[:-1, 1:], axis=-1
        )  # compare shifted images

        edges_bottom_right = horizontal_lines & vertical_lines
        edges_top_left = np.roll(horizontal_lines, 1, 0) & np.roll(
            vertical_lines, 1, 1
        )  # lines are too short -> roll to create intersections

        edge_coords = np.where(
            edges_bottom_right | edges_top_left
        )  # gather all edge coordinates

        def color_at(coords):
            return tuple(image[coords[0], coords[1]])

        bboxes = []
        for color, corner_points in groupby(
            sorted(zip(*edge_coords), key=color_at), key=color_at
        ):
            ...
            if len(corner_points) == 2 or allow_disconnected:
                bboxes.append(
                    BoundingBox.from_points(
                        corner_points, name=name_map[color], color=color
                    )
                )
                continue
            ...
            areas = [
                (np.count_nonzero(labels_im == label), label)
    ...
    def expand_children(
        ...
        if len(child_bboxes) == 1 and not parent_bbox.take_patch(occlusion_mask).any():
            return [
                BoundingBox(
                    top=parent_bbox.top,
                    left=parent_bbox.left,
                    height=parent_bbox.height,
                    width=parent_bbox.width,
                    name=child_bboxes[0].name,
                )
            ]

        shrink_y = max(
            min(child_bbox.height for child_bbox in child_bboxes) // 2 - 1, 0
        )
        shrink_x = max(min(child_bbox.width for child_bbox in child_bboxes) // 2 - 1, 0)
        grow_bboxes = [
            child_bbox.shrink(shrink_y, shrink_x) for child_bbox in child_bboxes
        ]
        ...
        def is_valid(bbox):
            ...
            if any(
                bbox.intersects(other) for other in grow_bboxes if grow_bbox != other
            ):
                return False
        ...
        return list(map(BoundingBox.union, child_bboxes, grow_bboxes))
