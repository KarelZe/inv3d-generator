...
from faker.providers import bank, company, date_time, internet, phone_number
...
class InvoiceContent:
    def __init__(
        ...
        products = {
            str(idx): self.fake_product(
                style, identifier, is_padding=idx >= num_products, tax_rate=tax_rate
            )
        ...
        self.complete_data = {
            ...
            "invoice_due_date": InvoiceContent.format_date(
            ...
            "summary": self.fake_summary(
                products, shipping_tag, discount_tag=discount_tag, tax_rate=tax_rate
            ),
    ...
    def subset_data(self, queries: set[str], output_file: Optional[Path] = None):
        # build subset of "self.complete_data" defined by the combination of all queries
        result = {}
        for query in queries:
            tmp = dpath.util.search(
                self.complete_data,
                query,
                separator=".",
                afilter=lambda x: x is not None,
            )
        ...
        if (
            len(
                list(
                    dpath.util.search(result, "**.__str__", separator=".", yielded=True)
                )
            )
            > 0
        ):
        ...
        matches = list(
            dpath.util.search(result, "**.__val__", separator=".", yielded=True)
        )
        for path, value in matches:
            ...
            _, parent_value = next(
                iter(
                    dpath.util.search(result, parent_path, separator=".", yielded=True)
                )
            )
            assert len(parent_value) == 1, (
                "Value attributes '__val__' cannot have sibling data fields!"
            )
    ...
    def collapse_data(self, output_file: Optional[Path] = None):
        data = dict(
            dpath.util.search(
                self.complete_data, "**", separator=".", yielded=True, dirs=False
            )
        )

        data = {
            path.removesuffix(".__str__"): value
    ...
    def fake_beneficiary(self):
        ...
        while True:
            try:
                ...
                bic = BIC.from_bank_code(
                    country_code=iban.country_code, bank_code=iban.bank_code
                )
            except ValueError:
                pass
        ...
        return {
            ...
            "bank": {"name": self.fake.company(), "address": self.fake_address()},
        }
    ...
    def fake_company(self):
        return {"name": self.fake.company(), "slogan": self.fake.catch_phrase()}
        ...
        for _ in range(100):
            try:
                ...
                parsed_number = phonenumbers.parse(
                    number_str, "US"
                )  # default country is the USA
                if phonenumbers.is_valid_number(parsed_number):
                    result = {
                        "__str__": number_str,
                        "country_code": self.format_value(
                            parsed_number.country_code, format_string="{}"
                        ),
                        "national_number": self.format_value(
                            parsed_number.national_number, format_string="{}"
                        ),
                    }
                    if parsed_number.extension is not None:
                        result["extension"] = self.format_value(
                            int(parsed_number.extension), format_string="{}"
                        )
                    return result
                ...
                )  # phone_number does not support this locale (reset locale)
    ...
    def fake_name(self):
        def remove_suffix(dictionary: dict, suffix: str):
            dictionary = {
                key.removesuffix(suffix): value for key, value in dictionary.items()
            }
            return {
                key: remove_suffix(value, suffix) if isinstance(value, dict) else value
    ...
    def inject_faker(
        self, method_name: str, final_formatters: Optional[list[str]] = None
    ):
        ...
        def parse_inject(_self, text):
            ...
            def wrapper(matches):
                formatter = next(iter(matches.groups()))
                assert formatter not in mapping
                mapping[formatter] = _self._Generator__format_token(matches)
                return mapping[formatter]

            re_token = re.compile(r"{{\s*(\w+)(:\s*\w+?)?\s*}}")
            formatted = re_token.sub(wrapper, text)

            mapping["__str__"] = formatted
            mapping = {
                key: collection[value]
                if value in collection and key not in final_formatters
                else value
    ...
    def calc_due_date(self, invoice_date: date_time.date, payment_terms: dict):
        due_offset = self.payment_terms_data[
            self.payment_terms_data["Abbreviation"] == payment_terms["__val__"]
        ].iloc[0]["DueOffset"]

        if isinstance(due_offset, float) and math.isnan(due_offset):
            return None

        if isinstance(due_offset, int) or (
            isinstance(due_offset, str) and due_offset.isdigit()
        ):
    ...
    def fake_payment_terms(self):
        ...
        return {"__str__": terms.PaymentTerm, "__val__": terms.Abbreviation}

    @staticmethod
    def fake_invoice_number():
        return InvoiceContent.format_value(
            value=random.randint(1000, 1000000), format_string="#{}"
        )

    @staticmethod
    def fake_customer_id():
        return InvoiceContent.format_value(
            value=random.randint(100000, 10000000), format_string="{}"
        )
    ...
    def fake_product(
        ...
        description = (
            ...
            if description_style == "capitalize"
            else product.Description.title()
        )
        ...
        return {
            ...
            "unit_price": InvoiceContent.format_value(
                value=unit_price, format_string="{:.2f}"
            ),
            "tax_rate": InvoiceContent.format_value(
                value=tax_rate, format_string="{:.1%}"
            ),
    ...
    @staticmethod
    def fake_summary(
        products: dict, shipping_tag: bool, discount_tag: bool, tax_rate: float
    ):
        subtotal = round(
            sum(
                product["total"]["__val__"]
        ...
        discount = (
            round(random.uniform(0, min(subtotal, 100)), 2)
            if discount_tag and random.random() < 0.5
            else 0
        )
        ...
        return {
            "subtotal": InvoiceContent.format_value(
                value=subtotal, format_string="{:.2f}"
            ),
            "discount": InvoiceContent.format_value(
                value=discount, format_string="{:.2f}"
            ),
            "subtotal_less_discount": InvoiceContent.format_value(
                value=subtotal_less_discount, format_string="{:.2f}"
            ),
            "tax_rate": InvoiceContent.format_value(
                value=tax_rate, format_string="{:.1%}"
            ),
            "tax_total": InvoiceContent.format_value(
                value=tax_total, format_string="{:.2f}"
            ),
            "shipping": {
                "price": InvoiceContent.format_value(
                    value=shipping, format_string="{:.2f}"
                ),
                "method": random.choice(["UPS", "USPS", "DHL", "FedEx"]),
            },
            "balance": InvoiceContent.format_value(
                value=balance, format_string="$ {:.2f}"
            ),
        }
    ...
    def export_ground_truth(self, output_dir: Path, template_fields: list[BoundingBox]):
        ...
        self.subset_data(
            queries={bbox.name for bbox in template_fields},
            output_file=ground_truth_structure_file,
        )
        ...
        json_boxes = [
            to_json(bbox)
            for bbox in template_fields
            if bbox.name in self.all_attributes
        ]
        json_boxes = [
            json_bbox for json_bbox in json_boxes if json_bbox["value"] is not None
        ]
        json_boxes = sorted(json_boxes, key=operator.itemgetter("tag"))
    ...
    def visualize(json_file: Path, background_file: Path, output_file: Path):
        ...
        BoundingBox.visualize(
            bboxes=bboxes, input_file=background_file, output_file=output_file
        )
