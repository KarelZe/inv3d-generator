...
def convert_exr_to_npz(exr_file: Path, npz_file: Path, override: bool = False):
    ...
    params = {npz_file.stem: data}
    np.savez_compressed(str(npz_file), **params)


def check_array(
    data: np.ndarray, shape: Optional[tuple] = None, dtype: Optional[np.dtype] = None
):
    assert isinstance(data, np.ndarray), (
        f"Object is not a numpy array! Found type is {data.dtype}"
    )

    if shape is not None:
        assert data.shape == shape, (
            f"Array shapes do not match! Expected shape is {shape}. Found shape is {data.shape}."
        )

    if dtype is not None:
        assert data.dtype == dtype, (
            f"Array dtypes do not match! Expected dtype is {dtype}. Found dtype is {data.dtype}."
        )
