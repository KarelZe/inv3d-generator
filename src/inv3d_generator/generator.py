...
class Inv3DGenerator:
    RATIOS: ClassVar[dict[str, float]] = {"train": 0.7, "val": 0.15, "test": 0.15}
    ...
    def __init__(
        ...
        if resume:
            ...
            self.data_dir.mkdir()
            if "settings_file" in args.__dict__:
    ...
    def _create_tasks(self, args: argparse.Namespace):
        ...
        asset_dirs = {
            asset_dir.name: asset_dir.resolve()
            for asset_dir in assets_dir.glob("*")
            if asset_dir.is_dir()
        }

        template_files = list_files(
            asset_dirs["templates"], suffixes=[".htm"], recursive=True
        )
        logo_files = list_files(asset_dirs["logos"], suffixes=[".png"], recursive=True)
        env_files = list_files(
            asset_dirs["environments"], suffixes=[".hdr", ".exr"], recursive=True
        )
        ...
        print(f"INFO: Number of available templates: {len(template_files)}")
        print(f"INFO: Number of available logos: {len(logo_files)}")
        print(f"INFO: Number of available environments: {len(env_files)}")
    ...
    def _create_task_files(self, settings: dict, num_samples: int):
        ...
        with tqdm.tqdm(
            total=num_samples, desc="Creating tasks", smoothing=0
        ) as progress_bar:
    ...
    @classmethod
    def _gather_fonts(
        cls, fonts_base_dir: Path, max_styles: int = 5
    ) -> dict[str, list[Path]]:
        all_fonts = {
        ...
        font_split = {
            ...
            for split, font_names in split_items(
                items=list(all_fonts.keys()), ratios=cls.RATIOS
            ).items()
        }

        print(f"INFO: Number of available fonts: {len(all_fonts)}")
        print(
            f"INFO: Number of available font styles: {sum(map(len, font_split.values()))}"
        )
    ...
    def _gather_meshes(cls, mesh_base_dir: Path) -> dict[str, list[Path]]:
        pattern = (
            r"^((?P<crop_id>\d+)_)?((?P<mesh_id>\d+)_(?P<augmentation_id>\d+).obj)$"
        )
        ...
        if len(all_meshes) < 10:
            print(
                f"WARNING: Found only {len(all_meshes)} different mesh types! Splits will be made on file level!"
            )
            mesh_split = split_items(
                items=list(chain.from_iterable(all_meshes.values())), ratios=cls.RATIOS
            )
        else:
            mesh_split = {
                split: [
                ...
                for split, mesh_ids in split_items(
                    items=list(all_meshes.keys()), ratios=cls.RATIOS
                ).items()
            }
    ...
    def process_tasks(self, num_workers: int = 0, verbose: bool = False):
        ...
        if num_workers > 0:
            self._process_tasks_parallel(
                task_files=task_files, num_workers=num_workers, verbose=verbose
            )
        else:
    ...
    def _process_tasks_parallel(
        self, task_files: list[Path], num_workers: int, verbose: bool = False
    ):
        ...
        with concurrent.futures.ProcessPoolExecutor(
            ...
            futures = [
                executor.submit(self.process_task, task_file, verbose)
            ...
            try:
                with tqdm.tqdm(
                    desc="Creating dataset", total=len(futures), smoothing=0
                ) as progress_bar:
    ...
    def _process_tasks_sequentially(
        self, task_files: list[Path], verbose: bool = False
    ):
    ...
    def process_task(task_file: Path, verbose: bool = False):
        ...
        # TODO (XCG-1396): remove later.
        tex_file = Path(
            "/home/markus/git/inv3d-generator/docs/readme_files/flat_document.png"
        )
        ...
        Inv3DGenerator._export_summary(
            data=summary, base_dir=assets_dir, output_file=sample_dir / "details.json"
        )
    ...
    @staticmethod
    def _export_summary(
        data: dict, base_dir: Path, output_file: Optional[Path]
    ) -> dict:
