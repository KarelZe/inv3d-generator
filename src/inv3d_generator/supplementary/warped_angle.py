...
class WarpedAngle:
    ...
    def from_uv_file(uv_file: Path, resolution_bm: int) -> "WarpedAngle":
        bm = BackwardMapping.from_uv_file(
            uv_file=uv_file, resolution_bm=resolution_bm, extrapolate=False
        )
        bm_data = np.roll(
            bm.data.transpose(1, 0, 2), shift=1, axis=-1
        )  # added for back-compatibility. BM changed its format after writing this.
        ...
        if torch.isnan(angles).any():
            ...
            angles = torch.from_numpy(
                np.expand_dims(angles.transpose((2, 0, 1)), axis=0)
            )
    ...
    def calc_angles_torch(bm: torch.Tensor) -> torch.Tensor:
        ...
        return ReplicationPad2d(1)(angles)[:, :, 1:, 1:]
    ...
    def warp_grid_torch(grid: torch.Tensor, uv: torch.Tensor) -> torch.Tensor:
        ...
        uv.requires_grad_(
            requires_grad=False
        )  # UV map (forward map) is considered a constant
        ...
        warped_grid = F.grid_sample(
            input=grid, grid=uv_grid, align_corners=True
        )  # N, C=2, H, W
        return warped_grid.transpose(1, 2).transpose(2, 3)  # N, H, W, C=2
