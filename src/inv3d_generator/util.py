...
class Tee:
    def __init__(self, name, mode):
        with Path(name).open(mode=mode) as f:
            self.file = f
...
def list_dirs(
    search_dir: Path, recursive: bool = False, as_string: bool = False, glob_string="*"
):
    glob_function = search_dir.rglob if recursive else search_dir.glob
...
def split_items(items: list, ratios: dict[str, float]):
    ...
    train_indexes, val_and_test_indexes = train_test_split(
        all_indexes, train_size=ratios["train"]
    )
    val_indexes, test_indexes = train_test_split(
        val_and_test_indexes,
