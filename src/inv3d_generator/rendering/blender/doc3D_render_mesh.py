...
Updated for modern Blender API: March 2025
"""
...
def reset_blend():
    ...
    # Updated cursor location access
    bpy.context.scene.cursor.location = (0.0, 0.0, 0.0)

    # only worry about data in the startup scene
    for bpy_data_iter in (
        bpy.data.meshes,
        bpy.data.lights,  # Changed from lamps to lights
        bpy.data.images,
        bpy.data.materials,
    ):
...
def is_visible(mesh, cam):
    bm = bmesh.new()  # create an empty BMesh
    bm.from_mesh(mesh.data)
    cam_direction = cam.matrix_world.to_quaternion() @ Vector((
        0.0,
        0.0,
        -1.0,
    ))  # changed * to @
    ...
    for v in bm.verts:
        co_ndc = world_to_camera_view(
            bpy.context.scene, cam, mat_world @ v.co
        )  # Changed * to @
...
def select_object(ob):
    ...
    # Updated active object setting
    bpy.context.view_layer.objects.active = None
    ob.select_set(True)  # Changed from ob.select = True
...
def prepare_scene():
    ...
    if random.random() > 0.5:
        ...
        bpy.data.scenes[
            "Scene"
        ].view_settings.view_transform = "Standard"  # Changed from 'Default'
...
def prepare_rendersettings(resolution):
    ...
    bpy.data.scenes[
        ...
        100  # Note: may need to use render.film_transparent = False in newer versions
    )
...
def add_lighting(envp):
    ...
    if envp is not None:
        ...
        # Updated mapping node inputs for newer Blender versions
        if envp.endswith(".exr"):
            mapping.inputs["Rotation"].default_value[0] = random.uniform(0, math.tau)
            mapping.inputs["Rotation"].default_value[1] = random.uniform(0, math.tau)
        ...
        litpos = Vector((0, d, 0))
        eul = Euler((0, 0, 0), "XYZ")
        eul.rotate_axis("Z", random.uniform(0, math.pi))
        eul.rotate_axis("X", random.uniform(math.radians(45), math.radians(135)))
        litpos.rotate(eul)

        # Updated light creation
        bpy.ops.object.light_add(type="POINT", location=litpos)
        ...
        if not lamp_node:
            lamp_node = nodes.new(type="ShaderNodeEmission")
        if not output_node:
...
def reset_camera(mesh):
    ...
    while id < 50:
        ...
        st = (d - 2.3) / 1.0 * 15 + 5.0
        eul.rotate_axis(
            "Z", random.uniform(math.radians(-90 - st), math.radians(-90 + st))
        )
...
def page_texturing(mesh, texpath):
    ...
    # Changed from Diffuse to Principled BSDF for newer Blender versions
    ...
    links.new(
        texture_node.outputs[0], bsdf_node.inputs["Base Color"]
    )  # Updated input name

    bsdf_node.inputs["Base Color"].show_expanded = True
...
def prepare_no_env_render():
    ...
    scene.view_settings.view_transform = "Standard"  # Changed from 'Default'
...
def render_pass(obj, objpath, texpath, output_paths):
    ...
    # Updated render layer settings
    scene = bpy.data.scenes["Scene"]
    # In newer Blender, use view_layers instead of render.layers
    view_layer = bpy.context.view_layer
    view_layer.use_pass_uv = True  # Enable UV pass
    ...
    if save_blend_file:
        bpy.ops.wm.save_mainfile(
            filepath=str(output_paths["bld"]) + "/" + fn + ".blend"
        )
    ...
    # Updated: Render layers output index may be different in newer Blender
    # Find the UV output
    for i, output in enumerate(render_layers.outputs):
        if output.name == "UV":
            links.new(render_layers.outputs[i], file_output_node_uv.inputs[0])
            break
    else:
        # Fallback to the old index if UV output not found by name
        links.new(render_layers.outputs[4], file_output_node_uv.inputs[0])
...
def render_img(objpath, texpath, envpath, resolution, output_paths):
    ...
    render_pass(mesh, objpath, texpath, output_paths)
    return None
...
def main():
    ...
    output_paths = {
        output_path: output_base_dir / output_path for output_path in output_names
    }

    for output_path in output_paths.values():
        output_path.mkdir(exist_ok=True)  # Added exist_ok=True
